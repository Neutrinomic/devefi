...
module {
    public module Ledgers {
        ...
        public type Ledger = {
            id: Principal;
            mem: {
                #icrc: MU.MemShell<ICRCLedger.Mem.Ledger.V2.Mem>;
                #icp: MU.MemShell<ICPLedger.Mem.Ledger.V2.Mem>;
            };
            virtual_mem : MU.MemShell<Virtual.Mem>;
        };

        public func upgrade(from : MU.MemShell<V2.Ledgers.Mem>) : MU.MemShell<Mem> {
            
            MU.upgrade(from, func(a : V2.Ledgers.Mem) : Mem {
                {
                    ledgers = Vector.vals(a.ledgers) |> Iter.map<V2.Ledgers.Ledger, Ledger>(_,
                        func(e : V2.Ledgers.Ledger) : Ledger  {
                            let mem = switch(e.mem) {
                                case (#icrc(m)) {
                                    #icrc(ICRCLedger.Mem.Ledger.V2.upgrade(m));
                                };
                                case (#icp(m)) {
                                    #icp(m);
                                };
